...
uki_reader.view.feeds.renderer = uki.newClass(uki.more.view.treeList.Render, function(Base) {
  ...
  this._leafTemplate = new uki.theme.Template(
    '<div class="${classPrefix}-row" style="margin-left:${indent}px; padding-right: ${unreadPadding}px">' +
    '<span style="float: left; background:url(${icon}) no-repeat -35px -3px; width: 13px; height: 13px; margin-right: 4px;"></span>' +
  ...
  var style = new uki.theme.Template(
    ...
    '.${classPrefix}-unread { position: absolute; padding: 2px 4px; min-width: 14px; text-align: center; right: 8px; top: 3px; color: #FFF; background: #94A7C8; -webkit-border-radius: 10px; -moz-border-radius: 10px; border-radius: 10px; line-height: 10px; font-size: 10px; font-weight: bold;  } ' +
    '.${classPrefix}-row { color: #000; position:relative; padding: 4px 3px 3px 10px; white-space: nowrap; text-overflow: ellipsis; *zoom: 1; } ' +
  ...
  var text = row.title(), children = uki.attr(row, 'children');
  var options = {
    text: row.title(), 
    unread: row.unread(),
    ...
    icon: 'http://www.google.com/reader/ui/1007736847-lhn-sprite.png',
    classPrefix: this.classPrefix
  };
